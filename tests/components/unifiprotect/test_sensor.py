...
async def test_sensor_setup_disk(
    hass: HomeAssistant,
    entity_registry: er.EntityRegistry,
    ufp: MockUFPFixture,
) -> None:
    ...
    # Setup NVR
    nvr: NVR = ufp.api.bootstrap.nvr
    assert nvr.system_info.ustorage is not None

    disk = nvr.system_info.ustorage.disks[0]
    disk.type = "SSD"
    disk.life_span = 94
    disk.temperature = 58
    disk.bad_sector = 2
    disk.power_on_hours = 1000
    disk.state = "normal"

    await init_entry(hass, ufp, [])

    assert_entity_counts(hass, Platform.SENSOR, 16, 9)

    # Check for the specific SSD sensors
    disk_life_span_entity = f"sensor.{nvr.mac.lower()}_disk_1_life_span"
    disk_temp_entity = f"sensor.{nvr.mac.lower()}_disk_1_temperature"
    disk_bad_sector_entity = f"sensor.{nvr.mac.lower()}_disk_1_bad_sector"
    disk_power_on_hours_entity = f"sensor.{nvr.mac.lower()}_disk_1_power_on_hours"

    # Check entity registry
    entity = entity_registry.async_get(disk_life_span_entity)
    assert entity
    assert entity.disabled is True
    assert entity.unique_id == f"{nvr.mac}_disk_1_life_span"

    entity = entity_registry.async_get(disk_temp_entity)
    assert entity
    assert entity.disabled is True
    assert entity.unique_id == f"{nvr.mac}_disk_1_temperature"

    entity = entity_registry.async_get(disk_bad_sector_entity)
    assert entity
    assert entity.disabled is True
    assert entity.unique_id == f"{nvr.mac}_disk_1_bad_sector"

    entity = entity_registry.async_get(disk_power_on_hours_entity)
    assert entity
    assert entity.disabled is True
    assert entity.unique_id == f"{nvr.mac}_disk_1_power_on_hours"

    await enable_entity(hass, ufp.entry.entry_id, disk_life_span_entity)
    await enable_entity(hass, ufp.entry.entry_id, disk_temp_entity)
    await enable_entity(hass, ufp.entry.entry_id, disk_bad_sector_entity)
    await enable_entity(hass, ufp.entry.entry_id, disk_power_on_hours_entity)

    assert hass.states.get(disk_life_span_entity).state == "94"
    assert hass.states.get(disk_temp_entity) == "58.0"
    assert hass.states.get(disk_bad_sector_entity) == "2"
    assert hass.states.get(disk_power_on_hours_entity) == "1000"
